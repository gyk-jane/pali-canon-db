...
import os
import ast
from scripts.fetch_sc_data import get_menu_data
...
def load_suttaplex_to_db(basket: str) -> None:
    ...
    path = f'data/processed/{basket}.json'
...
def load_children_to_db(basket: str) -> None:
    ...
    path = 'data/processed/'
    if f'{basket}_children.csv' in os.listdir(path):
        children_df = pd.read_csv(f'{path}{basket}_children.csv', quotechar='"')
    else:
...
def load_collections_to_db() -> None:
    ...
    """
    PATHS = ['data/raw/html_text.data.json',
             'data/raw/sc_bilara_texts.data.json']
    conn = connect_to_preprocess_db()
    for path in PATHS:
        with open(path, encoding='utf-8') as f:
        ...
        df.to_sql(path.split('/')[2].split('.')[0], conn, 
